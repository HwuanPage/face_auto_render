...
@click.option('--num-steps',              help='Number of optimization steps', type=int, default=10, show_default=True)
@click.option('--num-steps-pti',          help='Number of optimization steps for pivot tuning', type=int, default=100, show_default=True)
@click.option('--seed',                   help='Random seed', type=int, default=666, show_default=True)
@click.option('--outdir',                 help='Where to save the output images', required=True, metavar='DIR')
@click.option('--shapes', type=bool,      help='Gen shapes for shape interpolation', default=True, show_default=True)
@click.option('--extension',              help='3D file Format', type=click.Choice(['.mrc', '.ply']), default='.ply')
def run_projection(
    network_pkl: str,
    target_img:str,
    idx: int,
        ...
        with tqdm(total = samples.shape[1]) as pbar:
            with torch.no_grad():
                while head < samples.shape[1]:
                    torch.manual_seed(0)
                    # 개인 이미지에 대한 shape 생성// sample과 다른 부분
